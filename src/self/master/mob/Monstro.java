...
import self.master.item.Dropper;
...
public abstract class Monstro extends Mob {
	...
	protected void setRandomXY() {
		int novoX = 1 +(int)(Math.random() * (GameWindow.getWidthLQ() - width -5));
		int novoY = 1 +(int)(Math.random() * (GameWindow.getHeightLQ()- height - 5));
		...
		while(distancia < 150) {
			novoX = 1 +(int)(Math.random() * (GameWindow.getWidthLQ() - width));
			novoY = 1 +(int)(Math.random() * (GameWindow.getHeightLQ() - height));
